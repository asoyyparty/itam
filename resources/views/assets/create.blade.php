...
<div class="asset-form-card">
    ...
    <!-- 📄 Auto-Scan Dokumen & OCR (Input Aset 1-Klik) Widget -->
    <div class="ocr-scan-widget p-4 mb-4" style="background: color-mix(in oklch, var(--color-accent-tint) 15%, var(--color-paper-0)); border: 1px dashed var(--color-accent); border-radius: var(--radius-lg); margin: 20px 24px 0;">
        <div class="d-flex align-items-center justify-content-between mb-2">
            <div class="d-flex align-items-center" style="gap: 12px;">
                <div class="ocr-badge-icon" style="width: 42px; height: 42px; background: var(--color-accent-tint); border-radius: 12px; display: flex; align-items: center; justify-content: center; color: var(--color-accent); font-size: 1.2rem;">
                    <i class="fas fa-camera"></i>
                </div>
                <div>
                    <h5 class="font-weight-bold theme-text mb-0" style="font-size: 1rem;">
                        {{ __('messages.ocr_title') }}
                    </h5>
                    <small class="text-muted">{{ __('messages.ocr_subtitle') }}</small>
                </div>
            </div>
            <span class="badge badge-info px-3 py-2" style="font-size: 0.8rem; border-radius: 20px;"><i class="fas fa-sparkles mr-1"></i> AI Powered</span>
        <input type="file" id="ocrFileInput" accept="image/*,.pdf" style="display: none;">
        <div class="ocr-dropzone mt-3 text-center p-3" id="ocrDropzone" style="border: 2px dashed var(--color-accent-soft); border-radius: var(--radius-md); background: var(--color-paper-1); cursor: pointer; transition: background .2s;">
            <div id="ocrInitialState">
                <i class="fas fa-cloud-upload-alt text-info mb-2" style="font-size: 2rem;"></i>
                <p class="theme-text font-weight-bold mb-1">{{ __('messages.ocr_dropzone_title') }}</p>
                <small class="text-muted">{{ __('messages.ocr_dropzone_formats') }}</small>
            </div>
            <div id="ocrLoadingState" style="display: none;">
                <i class="fas fa-spinner fa-spin text-info mb-2" style="font-size: 2rem;"></i>
                <p class="theme-text font-weight-bold mb-1">{{ __('messages.ocr_loading_title') }}</p>
                <small class="text-muted">{{ __('messages.ocr_loading_subtitle') }}</small>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('assets.store', request()->query()) }}" method="POST">
        ...
        <div class="form-section">
            <div class="section-title section-purple">
                ...
                <div>
                    <div class="section-name">{{ __('messages.classification_location') }}</div>
        ...
        <div class="form-section">
            <div class="section-title section-green">
                ...
                <div>
                    <div class="section-name">{{ __('messages.acquisition') }}</div>
                    <div class="section-sub">{{ __('messages.date_received') }}, DO, {{ __('messages.warranty_months') }}, {{ __('messages.status') }}, {{ __('messages.notes') }}</div>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-3 form-group">
                    <label class="field-label">{{ __('messages.warranty_months') }}</label>
                    <div class="input-wrap">
                        <span class="input-icon"><i class="fas fa-shield-alt"></i></span>
                        <input type="number" name="warranty_months"
                            class="field-input @error('warranty_months') is-invalid @enderror"
        ...
        <div id="specifications-section" style="display:none;">
            <div class="form-section">
                <div class="section-title section-amber">
                    ...
                    <div>
                        <div class="section-name">{{ __('messages.category_specifications') }}</div>
...
.badge-cyan   { background: rgba(99, 102, 241, 0.15); color: #818cf8; }
.badge-purple { background: rgba(168, 85, 247, 0.15); color: #c084fc; }
.badge-green  { background: rgba(16, 185, 129, 0.15); color: #34d399; }
.badge-amber  { background: rgba(245, 158, 11, 0.15); color: #fbbf24; }
.section-name {
    font-size: 0.925rem; font-weight: 700; color: #f8fafc;
...
.field-input::placeholder { color: #64748b; font-size: 12px; }
...
/* ─── Mobile View Responsive Enhancements ──────────────── */
@media (max-width: 767.98px) {
    .ocr-scan-widget {
        margin: 14px 14px 0 !important;
        padding: 16px !important;
    }
    .ocr-scan-widget .d-flex.justify-content-between {
        flex-direction: column !important;
        align-items: flex-start !important;
        gap: 10px !important;
    }
    .ocr-scan-widget .badge {
...
$(document).ready(function () {
    ...
    // OCR Auto-Scan Document Handler (Event Delegation)
    $(document).on('click', '#ocrDropzone', function(e) {
        document.getElementById('ocrFileInput').click();
    });

    $(document).on('dragover dragenter', '#ocrDropzone', function(e) {
        e.preventDefault();
        e.stopPropagation();
        $(this).css('background', 'var(--color-accent-tint)');
    });

    $(document).on('dragleave drop', '#ocrDropzone', function(e) {
        e.preventDefault();
        e.stopPropagation();
        $(this).css('background', 'var(--color-paper-1)');
    });

    $(document).on('drop', '#ocrDropzone', function(e) {
        const files = e.originalEvent.dataTransfer.files;
        if (files.length > 0) {
            processOcrScanFile(files[0]);
        }
    });

    $(document).on('change', '#ocrFileInput', function() {
        if (this.files && this.files.length > 0) {
            processOcrScanFile(this.files[0]);
        }
    });

    function processOcrScanFile(file) {
        const formData = new FormData();
        formData.append('document', file);
        formData.append('_token', '{{ csrf_token() }}');

        $('#ocrInitialState').hide();
        $('#ocrLoadingState').show();

        $.ajax({
            url: "{{ route('assets.scan-ocr') }}",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                ...
                if (response.success && response.data) {
                    ...
                    if (d.name) $('input[name="name"]').val(d.name).trigger('input');
                    if (d.serial_number) $('input[name="serial_number"]').val(d.serial_number).trigger('input');
                    if (d.delivery_order_number) $('input[name="delivery_order_number"]').val(d.delivery_order_number).trigger('input');
                    if (d.date_received) $('input[name="date_received"]').val(d.date_received).trigger('input');
                    if (d.warranty_months) $('input[name="warranty_months"]').val(d.warranty_months).trigger('input');
                    if (d.notes) $('textarea[name="notes"]').val(d.notes).trigger('input');

                    if (d.category_id) {
                        $('select[name="category_id"]').val(d.category_id).trigger('change');
                    }
                    if (d.brand_id) {
                        $('select[name="brand_id"]').val(d.brand_id).trigger('change');
                    }

                    // Computer / Laptop Specs
                    if (d.cpu) $('input[name="cpu"]').val(d.cpu);
                    if (d.ram) $('input[name="ram"]').val(d.ram);
                    if (d.ssd) $('input[name="ssd"]').val(d.ssd);
                    if (d.hdd) $('input[name="hdd"]').val(d.hdd);
                    if (d.os) $('input[name="os"]').val(d.os);

                    // Flash Highlight Feedback
                    const fieldsToHighlight = $('input, select, textarea');
                    fieldsToHighlight.css('box-shadow', '0 0 12px #10b981');
                    setTimeout(() => {
                        fieldsToHighlight.css('box-shadow', '');
                    }, 2000);
                }
            },
            error: function(xhr) {
