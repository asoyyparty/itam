...
@section('title', __('messages.asset_details') . ' - ' . $asset->asset_tag)
...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="card theme-card text-center pb-3">
            <div class="card-body">
                ...
                <div class="text-left">
                    <p class="theme-text mb-2"><strong>{{ __('messages.category') }}:</strong> <span class="float-right text-info">{{ $asset->category->name ?? '-' }}</span></p>
                    <p class="theme-text mb-2"><strong>{{ __('messages.brand') }}:</strong> <span class="float-right text-info">{{ $asset->brand->name ?? '-' }}</span></p>
                    <p class="theme-text mb-2"><strong>{{ __('messages.current_location') }}:</strong> <span class="float-right text-info">{{ $asset->location->name ?? '-' }}</span></p>
            ...
            <div class="px-3">
                @auth
                <a href="{{ route('assets.edit', $asset) }}" class="btn btn-block btn-info mb-2"><i class="fas fa-edit"></i> {{ __('messages.edit_asset') }}</a>
                <a href="{{ route('assets.index') }}" class="btn btn-block btn-outline-secondary"><i class="fas fa-arrow-left"></i> {{ __('messages.back_to_catalog') }}</a>
                @endauth
            </div>
        </div>

        <!-- Predictive Health AI Card -->
        @php
            $health = app(\App\Services\PredictiveHealthService::class)->calculateAssetHealth($asset);
        @endphp
        <div class="card theme-card mt-3" style="border-left: 4px solid {{ $health['text_color'] }} !important;">
            <div class="card-header d-flex justify-content-between align-items-center py-3">
                <h5 class="card-title m-0 theme-text"><i class="fas fa-heartbeat text-danger mr-2"></i> {{ __('messages.health_score_ai') }}</h5>
                <span class="badge {{ $health['badge_class'] }} px-2 py-1">{{ $health['status'] }}</span>
            </div>
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <span class="theme-text font-weight-bold">{{ __('messages.health_score_label') }}:</span>
                    <strong style="font-size: 1.4rem; color: {{ $health['text_color'] }};">{{ $health['health_score'] }}%</strong>
                </div>
                <div class="progress mb-3" style="height: 8px; background: rgba(255,255,255,0.08); border-radius: 10px;">
                    <div class="progress-bar" style="width: {{ $health['health_score'] }}%; background-color: {{ $health['text_color'] }}; border-radius: 10px;"></div>
                </div>

                <p class="theme-text small mb-2"><strong>{{ __('messages.remaining_life') }}:</strong> <span class="float-right text-info">{{ $health['remaining_life'] }}</span></p>

                <hr style="border-top: 1px solid var(--tech-border);">
                <small class="text-muted d-block mb-1 font-weight-bold">{{ __('messages.ai_recommendation') }}:</small>
                <p class="theme-text small mb-0">{{ $health['recommendation'] }}</p>

                <div class="mt-3">
                    <a href="{{ route('predictive-health.index') }}" class="btn btn-sm btn-block btn-outline-info">
                        <i class="fas fa-sparkles mr-1"></i> {{ __('messages.predictive_health') }}
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="card theme-card mt-3">
            <div class="card-header">
                <h5 class="card-title m-0 theme-text"><i class="fas fa-network-wired text-success mr-2"></i> {{ __('messages.network_ip') }}</h5>
            </div>
            <div class="card-body p-0">
                <ul class="list-group list-group-flush theme-list" style="background: transparent;">
                    ...
                    @empty
                        <li class="list-group-item text-center text-muted" style="background: transparent; border: none;">{{ __('messages.no_ips_linked') }}</li>
                    @endforelse
                </ul>
            </div>
        </div>
    ...
    <div class="col-md-8">
        ...
        <div class="card theme-card">
            <div class="card-header">
                <h5 class="card-title m-0 theme-text"><i class="fas fa-info-circle text-info mr-2"></i> {{ __('messages.specifications_info') }}</h5>
            </div>
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-sm-6">
                ...
                @if(in_array($catName, ['komputer', 'laptop', 'mini pc', 'thin client', 'server']) && $asset->computer)
                    <hr style="border-top: 1px solid var(--tech-border);">
                    <h5 class="text-info mb-3"><i class="fas fa-microchip mr-2"></i> {{ __('messages.hardware_specs') }}</h5>
                    <div class="row">
                ...
                @if($catName == 'printer' && $asset->printer)
                    <hr style="border-top: 1px solid var(--tech-border);">
                    <h5 class="text-info mb-3"><i class="fas fa-print mr-2"></i> {{ __('messages.printer_parameters') }}</h5>
                    <div class="row">
                        <div class="col-sm-6 mb-3">
                            <div class="d-flex justify-content-between align-items-start border-bottom pb-2" style="border-bottom: 1px solid var(--tech-border) !important;">
                ...
                @if($catName == 'monitor' && $asset->monitor)
                    <hr style="border-top: 1px solid var(--tech-border);">
                    <h5 class="text-info mb-3"><i class="fas fa-desktop mr-2"></i> {{ __('messages.monitor_properties') }}</h5>
                    <div class="row">
                        <div class="col-sm-6 mb-3">
                            <div class="d-flex justify-content-between align-items-start border-bottom pb-2" style="border-bottom: 1px solid var(--tech-border) !important;">
                ...
                @if(in_array($catName, ['switch', 'router', 'access point', 'firewall']) && $asset->networkDetail)
                    <hr style="border-top: 1px solid var(--tech-border);">
                    <h5 class="text-info mb-3"><i class="fas fa-network-wired mr-2"></i> {{ __('messages.network_parameters') }}</h5>
                    <div class="row">
                        <div class="col-sm-6 mb-3">
                            <div class="d-flex justify-content-between align-items-start border-bottom pb-2" style="border-bottom: 1px solid var(--tech-border) !important;">
                ...
                @if(in_array($catName, ['cctv', 'camera']) && $asset->cctv)
                    <hr style="border-top: 1px solid var(--tech-border);">
                    <h5 class="text-info mb-3"><i class="fas fa-video mr-2"></i> {{ __('messages.cctv_config') }}</h5>
                    <div class="row">
                        <div class="col-sm-6 mb-3">
                            <div class="d-flex justify-content-between align-items-start border-bottom pb-2" style="border-bottom: 1px solid var(--tech-border) !important;">
                ...
                @if($asset->category && $asset->category->spec_definitions && !empty($asset->spec_data))
                    <hr style="border-top: 1px solid var(--tech-border);">
                    <h5 class="text-info mb-3"><i class="fas fa-list mr-2"></i> {{ __('messages.custom_specifications') }}</h5>
                    <div class="row">
