    ...
    <form action="{{ route('password_vaults.store') }}" method="POST">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="text" name="device_name" class="form-control theme-input @error('device_name') is-invalid @enderror" value="{{ old('device_name') }}" placeholder="e.g., CCTV NVR Lab, Switch Core Server, Budi AnyDesk" required>
                @error('device_name') <span class="text-danger">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label class="theme-text">{{ __('messages.username') }} *</label>
                <input type="text" name="username" class="form-control theme-input @error('username') is-invalid @enderror" value="{{ old('username') }}" placeholder="e.g., admin, root, budi@itam.local" required>
