...
Route::middleware('auth')->group(function () {
    ...
    // AI Assistant & OCR Routes
    Route::middleware(['permission:action_ai_assistant'])->group(function () {
        Route::post('ai/analyze-ticket', [\App\Http\Controllers\AiController::class, 'analyzeTicket'])->name('ai.analyze-ticket');
        Route::post('ai/query-search', [\App\Http\Controllers\AiController::class, 'querySearch'])->name('ai.query-search');
    });
    Route::middleware(['permission:action_ocr_scan'])->group(function () {
        Route::post('assets/scan-ocr', [\App\Http\Controllers\OcrScanController::class, 'scan'])->name('assets.scan-ocr');
    });

    // Predictive Health & Maintenance AI Routes
    Route::middleware(['permission:menu_predictive_health'])->group(function () {
        Route::get('predictive-health', [\App\Http\Controllers\PredictiveHealthController::class, 'index'])->name('predictive-health.index');
        Route::get('predictive-health/{asset}/analyze', [\App\Http\Controllers\PredictiveHealthController::class, 'analyzeAsset'])->name('predictive-health.analyze');
    });

    // Network Anomaly Detection Routes
    Route::middleware(['permission:menu_network_anomalies'])->group(function () {
        Route::get('network-anomalies', [\App\Http\Controllers\NetworkAnomalyController::class, 'index'])->name('network-anomalies.index');
        Route::post('network-anomalies/resolve', [\App\Http\Controllers\NetworkAnomalyController::class, 'resolve'])->name('network-anomalies.resolve');
        Route::post('network-anomalies/register-rogue', [\App\Http\Controllers\NetworkAnomalyController::class, 'registerRogue'])->name('network-anomalies.register-rogue');
    });

    // IT Budget & Lifecycle Planning Routes
    Route::middleware(['permission:menu_budget_planner'])->group(function () {
        Route::get('budget-planner', [\App\Http\Controllers\BudgetPlannerController::class, 'index'])->name('budget-planner.index');
        Route::get('budget-planner/export', [\App\Http\Controllers\BudgetPlannerController::class, 'exportExcel'])->name('budget-planner.export');
    });
