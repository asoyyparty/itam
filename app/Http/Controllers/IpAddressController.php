...
class IpAddressController extends Controller implements \Illuminate\Routing\Controllers\HasMiddleware
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('vlan_id') && $request->vlan_id != '') {
    ...
    public function create()
    {
        ...
        $vlans = \Illuminate\Support\Facades\Cache::remember('master_vlans_all', 3600, fn() => Vlan::orderBy('vlan_number')->get());
    ...
    public function agentSync(Request $request)
    {
        ...
        foreach ($statuses as $item) {
            ...
            if (!empty($item['id'])) {
                $targetIp = IpAddress::find($item['id']);
            } elseif (!empty($item['ip'])) {
                $targetIp = IpAddress::where('ip_address', $item['ip'])->first();
            }
