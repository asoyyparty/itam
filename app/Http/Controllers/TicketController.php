...
class TicketController extends Controller implements \Illuminate\Routing\Controllers\HasMiddleware
{
    ...
    public function store(Request $request)
    {
        ...
        // Trigger Telegram & WhatsApp Push Alert
        try {
            app(\App\Services\TelegramBotService::class)->sendNewTicketNotification($ticket);
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Telegram ticket alert error: ' . $e->getMessage());
        }

        try {
            app(\App\Services\WhatsAppService::class)->sendNewTicketNotification($ticket);
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('WhatsApp ticket alert error: ' . $e->getMessage());
        }
